...
export default async function IncomeCategoriesPage() {
  ...
  const rows = categories.filter((c) => c.direction === "in");
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Откуда компания получает деньги"
        actions={
          <AddRecord
            title="Новая категория дохода"
            label="Новая категория"
            action={createIncomeCategory}
            submitLabel="Добавить категорию"
            fields={[
              { name: "name", label: "Название", placeholder: "Например, Услуги", required: true },
            ]}
          />
        }
      />
